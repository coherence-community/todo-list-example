...
public class TaskApi
     ...
     */
    @Mutation
    @Description("Remove all completed tasks and return the number of tasks removed")
    public Integer deleteCompletedTasks()
        {
        Collection<String> keys = tasks.keySet(Filters.equal(Task::isCompleted, true));
        if (keys.size() > 0)
            {
            tasks.invokeAll(keys, Processors.remove(Filters.always()));
            }
        return keys.size();
        }
